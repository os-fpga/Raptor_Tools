...
TEST(ParameterTypeTest, IntEnum) {
    ...
    int_param.set_enum_value("One", 1);
    int_param.set_enum_value("Seven", 7);

    EXPECT_TRUE(int_param.has_enum_value("One"));
    EXPECT_EQ(int_param.get_enum_value("One"), 1);
    EXPECT_TRUE(int_param.has_enum_value("Seven"));
    EXPECT_EQ(int_param.get_enum_value("Seven"), 7);
    EXPECT_THROW(int_param.set_enum_value("Eight", 8), std::runtime_error);
}
...
TEST(ParameterTypeTest, IntDefaultEnum) {
    ...
    int_param.set_default_value("One");

    EXPECT_TRUE(int_param.has_default_value());
    EXPECT_EQ(int_param.get_default_value(), 1);
    EXPECT_THROW(int_param.set_default_value("Eight"), std::runtime_error);
}
