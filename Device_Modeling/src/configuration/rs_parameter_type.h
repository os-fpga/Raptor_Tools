...
public:
    ...
    /**
     * @brief Set the default value by enum name.
     * @param key The name of the enum value to set as the default value.
     * @throws std::runtime_error If the enum value is not found.
     */
    void set_default_value(std::string key)
    {
        if (enum_values_.find(key) == end(enum_values_))
        {
            throw std::runtime_error("Could not find the enumeration key " + key + "to set the default_value");
        }
        default_value_ = enum_values_[key];
    }
    ...
    void set_default_value(int value)
    {
        ...
        {
            throw std::runtime_error("Invalid default value \"" + std::to_string(value) + "\" as the parameter lower bound is set to " + std::to_string(get_lower_bound()));
        }
        if (has_upper_bound() && value > get_upper_bound())
        {
            throw std::runtime_error("Invalid default value \"" + std::to_string(value) + "\" as the parameter upper bound is set to " + std::to_string(get_upper_bound()));
        }
