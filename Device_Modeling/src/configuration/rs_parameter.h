...
public:
     ...
     */
    Parameter(const std::string &name, const T &value, const ParameterType<T> &type)
        : name_(name), value_(value), type_(type)
    {
        if (!type_.is_valid(value_))
        {
     ...
     */
    void set_type(const ParameterType<T> &type)
    {
        type_ = type;
    }
     ...
     */
    ParameterType<T> get_type() const
    {
        return type_;
    }
