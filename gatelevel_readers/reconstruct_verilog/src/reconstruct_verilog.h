...
public:
  ...
  void port_case_compare() {
    std::cout << "Start case diffs" << std::endl;
    std::unordered_set<std::string> diffs;
    for (auto &p : prim_io_maps_dsp_to_rs_prim) {
      for (auto &pp : p.second) {
        std::string name = pp.first;
        std::transform(name.begin(), name.end(), name.begin(), ::tolower);
        if (name != pp.second) {
          std::cout << "Warn in Module " << p.first << " : " << name << " / "
                    << pp.second << std::endl;
          diffs.insert(pp.second + " : " + pp.first);
        }
      }
    }
    ...
    std::cout << "End case diffs" << std::endl;
  }
