                    ...
                    std::string width)
{
  ...
  {
    int size;
    if (string_compare(std::get<3>(ports_vector[i]), "")) // if the input port size is not defined set it to zero
      size = 0;
    else
      size = stoi(std::get<3>(ports_vector[i])); // else get the size from the value
