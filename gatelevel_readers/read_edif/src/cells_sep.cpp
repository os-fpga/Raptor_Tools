...
// every time the keyword port is occurs in a cell this function is called and
// the available data is extracted from this node

void cells_sep::ports(struct SNode *head, std::string port_name,
                      struct cells *cell_) {
  struct SNode *current = head;
  std::string port_name_orig = port_name;
  std::string port_name_renamed = port_name;
  std::string port_direction, port_size;
  ...
  cell_->ports_vector.push_back(std::make_tuple(
      port_name_orig, port_name_renamed, port_direction, port_size));
}
...
void cells_sep::cells_sep::instances(struct SNode *head,
  ...
  bool is_lut = false;
  struct SNode *current = head;
  std::string inst_name_orig = instance_name;
  std::string inst_name_renamed = instance_name;
    ...
    if (current->type == LIST) {
      ...
      if (string_compare("rename", child->value)) {
        ...
        inst_name_orig = child->value;
                 ...
                 (string_compare("viewref", child->value))) {
        child = child->next;
        child = child->next;
        if (child->type == LIST) {
          struct SNode *n_child = child->list;
          if ((string_compare("cellRef", n_child->value)) ||
              (string_compare("cellref", n_child->value)))
            n_child = n_child->next;
  ...
  // instance_tuple = std::make_tuple(inst_name_orig, inst_name_renamed,
  // inst_cell_ref, prop_lut, prop_width, is_lut);
  cell_->instance_vector.push_back(
      std::make_tuple(inst_name_orig, inst_name_renamed, inst_cell_ref,
                      prop_lut, prop_width, is_lut));
}

// One net can connect to multiple pins of lut or sub circuits so a seperate
// mechanism is developed to get the net connection

void cells_sep::cells_sep::get_nets(
    struct SNode *head, std::string net_name, std::string net_name_remaned,
    struct cells *cell_) //, std::string net_name
{
  ...
  while (current != NULL) // iterate untill the joined does not end
  {
    if (current->type == LIST) {
      ...
      if ((string_compare("portRef", child->value)) ||
          (string_compare("portref", child->value))) {
        while (child !=
               NULL) // iterate in the net connection list aka (port ref)
        {
          if (child->type == LIST) {
            struct SNode *n_child = child->list;
            ...
            if ((string_compare("instanceRef", n_child->value)) ||
                (string_compare("instanceref", n_child->value))) {
              n_child = n_child->next;
        ...
        net_vector.push_back(std::make_tuple(
            net_port_ref, net_member, net_instance_ref, net_name_remaned));
...
/*
 * The get cell data seperate the nodes of ports, instances and the nets for
 * each cell
 *
 */
void cells_sep::cells_sep::get_cell_data(struct SNode *head,
  ...
  while (current != NULL) {
    ...
    if (current->type == SYMBOL) {
      ...
      /*The port can be either renamed or passed directly we have to get both
         the values to generate the blif*/
      {
        current = current->next;
